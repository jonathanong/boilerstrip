...
import { learn, convert, convertMany, type Removals } from '../index'
...
describe('learn', () => {
  ...
  it('throws when fewer than 2 pages are provided', async () => {
    await expect(learn([buf('<html></html>')])).rejects.toThrow()
  })
...
describe('convert', () => {
  ...
  it('converts without options', async () => {
    const html = buf('<html><body><main><h1>Hello</h1><p>World</p></main></body></html>')
    const result = await convert(html)
  ...
  it('applies text density filter via options', async () => {
    ...
    const html = buf(
      `<html><body><footer>${longFooter}</footer><article>Real content here</article></body></html>`,
    )
...
describe('convertMany', () => {
  it('processes multiple HTML buffers in one call', async () => {
    const htmls = [
      buf('<html><body><main><h1>Page One</h1><p>First content</p></main></body></html>'),
      buf('<html><body><main><h1>Page Two</h1><p>Second content</p></main></body></html>'),
      buf('<html><body><main><h1>Page Three</h1><p>Third content</p></main></body></html>'),
    ]
    const results = await convertMany(htmls)
    expect(results).toHaveLength(3)
    expect(results[0].content).toContain('Page One')
    expect(results[1].content).toContain('Page Two')
    expect(results[2].content).toContain('Page Three')
  })
  ...
  it('applies shared options to all documents', async () => {
    const htmls = [
      buf('<html><body><nav class="nav">Menu</nav><main>Doc One</main></body></html>'),
      buf('<html><body><nav class="nav">Menu</nav><main>Doc Two</main></body></html>'),
    ]
    const results = await convertMany(htmls, {
      cssSelectorsToRemove: ['.nav'],
    })
    expect(results[0].content).not.toContain('Menu')
    expect(results[1].content).not.toContain('Menu')
    expect(results[0].content).toContain('Doc One')
    expect(results[1].content).toContain('Doc Two')
  })
})
